...
const PHYLA = ['All', 'Beast', 'Bug', 'Constellation', 'Construct', 'Demon', 'Dude', 'Elemental', 'Elf', 'Goblin', 'Hippy', 'Hobo', 'Humanoid', 'Horror', 'Mer-Kin', 'Orc', 'Penguin', 'Pirate', 'Plant', 'Slime', 'Undead', 'Weird']
...
function PhylumChooser(availableMonsters: { [key: number]: MonsterData }): JSX.Element {
    ...
    const onMonsterUpdated = (): void => {
        ...
        let enchantments = [], image = 'data:,', adventures = 0, name = '', monster: MonsterData = {};
    ...
    function updateOptions(): void {
        const onlyFree = document.getElementById('freeFightOnly') ? document.getElementById('freeFightOnly').checked : false;
        const phylum = document.getElementById('phylum') ? document.getElementById('phylum').value.toLowerCase() : 'all';
    ...
    return (
        <div style={{ display: 'inline-flex' }}>
            <div>
                ...
                <div>
                    <label>
                        <input id="freeFightOnly" type="checkbox" onChange={updateOptions}></input>Free Fights only
                    </label>
                    <label>
                        Phylum:
                        <select id="phylum" onChange={updateOptions}>
                            {PHYLA.map((phylum) => (<option value={phylum}>{phylum}</option>))}
                        </select>
                    </label>
                </div>
            </div>
